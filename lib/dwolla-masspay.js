...
module.exports = function(config, logger) {
  ...
  var create_job = function(email, filedata, assumeCosts, test, source, user_job_id, callback) {
    logger.info('dwolla-masspay#create_job(): %d payments; notifying <%s>; test: %s; user_job_id: %s', 
    ...
    request.post({
      ...
      strictSSL: false, // currently isn't working with latest node version
      json: create_req
  ...
  var job_details_by_id = function(job_id, user_job_id, callback) {
    logger.info('dwolla-masspay#job_details_by_id(): job_id: %s; user_job_id: %s', 
      job_id, user_job_id);
    var status_req = {
      uid: config.uid
    };
